...
class Tatfidf():
    ...
    def init(self, n=0):
        ...
        cv = CountVectorizer(self.copus, tokenizer=self.tokenizer.vi_term_tokenize, lowercase=False, min_df=1,
                             max_df=0.85)
    ...
    def compute_ajust_time(self):
        ...
        for token in self.all_features:
            ...
            sd_x = np.std(vals_without_token, dtype=np.float64, ddof=1) if len(vals_without_token) > 0 else 0.0
